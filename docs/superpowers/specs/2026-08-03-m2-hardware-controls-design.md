...
| G2 | 权限 | G1 | 独立评审临时 ACL 或精确 udev 方案 | vendor-only、`0666`、未验证的 subsystem |
...
2. 在仓库中生成精确 `6602:1000` 的 USB 设备级惰性规则模板（已验证 subsystem +
   `TAG+="uaccess"`），并只做离线测试。规则只做设备级匹配：udev 规则的 `ATTRS{}`
   只能从事件设备加一个父设备取属性，接口属性无法与 idVendor/idProduct 同处一行，
   接口级批准由软件实施，不由 udev。
...
- 2026-08-03：产品负责人批准正式书面规格，并授权编写 G0 实施计划。
- 当前待办：完成并复核 G0 实施计划；尚未授权 G0 实施、SDK 激活、权限变更或
  硬件访问。
